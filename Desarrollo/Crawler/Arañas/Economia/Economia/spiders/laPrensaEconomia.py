...
class procesoSpider(CrawlSpider):
    name = 'economiaLaPrensa'
    item_count = 0
    dateToday = datetime.datetime.now().strftime("%d-%m-%y-%H:%M")
    allowed_domain = ['https://www.la-prensa.com.mx/finanzas/']
    start_urls = ['https://www.la-prensa.com.mx/finanzas/']

    rules = {

          Rule(LinkExtractor(allow=(), restrict_xpaths=('//div[@class="column-heading load-more btn cargar-mas-button"]'))),
          Rule(LinkExtractor(allow=(), restrict_xpaths=('//h4/a | //ul[@class="list-unstyled"]/li/a')),
